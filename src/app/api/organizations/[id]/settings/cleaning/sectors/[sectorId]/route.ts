import { deleteSector, upsertSector } from "@/features/settings/lib/cleaning-data";
import { cleaningSectorSchema } from "@/features/settings/schemas";
...
export async function PATCH(
  request: Request,
  ctx: RouteContext<"/api/organizations/[id]/settings/cleaning/sectors/[sectorId]">,
) {
  try {
    ...
    const parsed = cleaningSectorSchema.safeParse(body);
    if (!parsed.success) {
      return jsonError(400, parsed.error.issues[0]?.message ?? "Dados inválidos.");
    }

    const result = await upsertSector(id, parsed.data, sectorId);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await deleteSector(sectorId, id);
