import { getWeeklySectors, upsertWeeklySector } from "@/features/settings/lib/cleaning-data";
...
export async function GET(
  request: Request,
  ctx: RouteContext<"/api/organizations/[id]/settings/cleaning/weekly-sectors">,
) {
  try {
    ...
    const sectors = await getWeeklySectors(id);
...
export async function POST(
  ...
  try {
    ...
    const result = await upsertWeeklySector(id, parsed.data);
